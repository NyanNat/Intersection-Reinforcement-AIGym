...
# Implementation of Deep Q Learning on traffic control problem
...
class DeepQNetwork(nn.Module):
    def __init__(self, obs_state_size, discrete_action_size, binary_action_size):
        super(DeepQNetwork, self).__init__()
        self.fc = nn.Sequential(
            nn.Linear(obs_state_size, 1024),
            nn.ReLU(),
            nn.Linear(1024, 4096),
            nn.ReLU(),
            nn.Linear(4096, 4096),
        ...
        self.fc_binary = nn.Linear(128, binary_action_size)
        self.fc_discrete = nn.Linear(128, discrete_action_size)

    def forward(self, state):
        x = torch.cat(state, dim=-1)
        x = self.fc(x)
        x_binary = torch.sigmoid(self.fc_binary(x)).clone()
        x_discrete = torch.softmax(self.fc_discrete(x), dim=-1).clone()
        return x_binary, x_discrete

class DeepQLearning:
    def __init__(self, alpha, gamma, epsilon, epsilon_min):
       self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
       binary_action_size = 10
       ...
       self.epsilon_decay = 0.987
       self.model = DeepQNetwork(observation_size, discrete_action_size, binary_action_size).to(self.device)
       self.target_model = DeepQNetwork(observation_size, discrete_action_size, binary_action_size).to(self.device)
       self.model.train(mode=True)
       self.target_model.train(mode=True)
       self.model.eval()
       self.target_model.eval()
       self.target_model.load_state_dict(self.model.state_dict())
       self.optimizer = optim.Adam(self.model.parameters(), lr=0.001)

    def appendExperienceReplay(self, state, action, reward, next_state, done):
        state = [torch.FloatTensor(component).to(self.device) for component in state]
        next_state = [torch.FloatTensor(component).to(self.device) for component in next_state]
        if any(torch.isnan(component).any() or torch.isinf(component).any() for component in state + next_state):
            print("NaN or Infinity detected in state or next_state. Skipping experience.")
            return

        # Print debug information
        print("State shapes:", [component.shape for component in state])
        print("Action:", action)
        print("Reward:", reward)
        print("Next State shapes:", [component.shape for component in next_state])
        print("Done:", done)
    ...
    def epsilonGreedy(self, state):
        if np.random.rand() <= self.epsilon:
            ...
            state = [torch.FloatTensor(component).to(self.device) for component in state]
            binary_output, discrete_output = self.model(state)
            binary_action = torch.bernoulli(binary_output).detach().cpu().numpy().astype(int)
            discrete_action = torch.argmax(discrete_output).item()
            return binary_action, discrete_action
    ...
    def experienceReplay(self, batch_size):
        ...
        states, binary_targets, discrete_targets = [], [], []

        for state, action, reward, next_state, done in minibatch:
            binary_target, discrete_target = self.model(state)

            predicted_next_binary_output, predicted_next_discrete_output = self.target_model(next_state)
            binary_target += self.alpha * (reward + self.gamma * torch.max(predicted_next_binary_output).detach().cpu().numpy() - binary_target)
            discrete_target += self.alpha * (reward + self.gamma * torch.max(predicted_next_discrete_output).detach().cpu().numpy() - discrete_target)

            states.append(state)
            binary_targets.append(binary_target)
            discrete_targets.append(discrete_target)

        binary_targets = [torch.FloatTensor(binary_target).to(self.device) for binary_target in binary_targets]
        discrete_targets = [torch.FloatTensor(discrete_target).to(self.device) for discrete_target in discrete_targets]

        torch.autograd.set_detect_anomaly(True)
        self.optimizer.zero_grad()
        for index, state in enumerate(states):
            binary_output, discrete_output = self.model(state)
            binary_target = normalize(binary_targets[index], p=2, dim=0)
            discrete_target = normalize(discrete_targets[index], p=2, dim=0)
            #binary_output_target = normalize(binary_output, p=2, dim=0)
            #discrete_output_target = normalize(binary_output, p=2, dim=0)

            binary_loss = nn.BCELoss()(binary_output, binary_target)
            discrete_loss = nn.CrossEntropyLoss()(discrete_output, discrete_target)
...
if __name__ == "__main__":
    ...
    env = TrafficControlEnv(END_TIME=90)
    state = env.reset()
    total_reward = 0
    dqn_agent = DeepQLearning(alpha_, gamma_, epsilon_, EPS_MIN)

    reward_array = []
    cars_passed_array = []
    episodes_array = np.arange(1, episodes+1)

    for i in range(episodes):
        print(f"Episode {i+1} started !")
        
        while True:
            action = dqn_agent.epsilonGreedy(state)
            observation, reward, done, _, cars_passed = env.step(action)
            next_state = observation
            dqn_agent.appendExperienceReplay(state, action, reward, next_state, done)
            state = next_state
            total_reward += reward

            if done:
                dqn_agent.target_train()
                print(f"Episode: {i + 1}/{episodes}, Total Reward: {total_reward}, Epsilon: {dqn_agent.epsilon}, Num. of Cars Passed: {cars_passed}")
                reward_array.append(reward)
                cars_passed_array.append(cars_passed)
                dqn_agent.experienceReplay(BATCH_SIZE)
                dqn_agent.decay_epsilon()
                next_state = env.reset()
                break
    
    plt.plot(episodes_array, reward_array, label='Reward')
    plt.plot(episodes_array, cars_passed_array, label='Num of Cars Passed')

    plt.xlabel('Episodes')
    plt.ylabel('Value')
    plt.title('Reward and Number of Cars Passed Plot')
